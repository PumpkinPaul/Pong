...
public class DebugCommandUI : DrawableGameComponent, IDebugCommandHost
{
    ...
    // Registered commands
    private readonly Dictionary<string, CommandInfo> _commandTable = new Dictionary<string, CommandInfo>();
    ...
    public void ProcessKeyInputs(KeyboardState keyState, int index, float dt)
    {
        ...
        foreach (var key in keys)
        {
            ...
            char ch;
            if (KeyboardUtils.KeyToString(key, shift, chatPadGreen, chatPadOrange, out ch))
            {
