...
public class Plotter
{
    ...
    private readonly Dictionary<string, Plot> _plots = new Dictionary<string, Plot>();
    ...
    private void GetLimits(float[] values, out float min, out float max)
    {
        max = float.MinValue;
        min = float.MaxValue;
    ...
    public void DrawLines(string name, float[] values, Vector2 position, Vector2 size, int decimalPlaces, Color lineColor)
    {
        ...
        GetLimits(values, out actualMin, out actualMax);
